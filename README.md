...
|-------|------|-------------|
| `/harness` | user-invoked | 파이프라인 시작/재개/상태 |
| `/harness --status` | user-invoked | 진행 상황 표시 |
| `/harness --resume` | user-invoked | 일시정지 후 재개 |
| `harness-research` | role | 시장 조사 + 경쟁 분석 |
| `harness-plan` | role | PRD 작성 |
| `harness-design` | role | 디자인 시스템 |
| `harness-contract` | role | 완료 기준 협상 |
| `harness-generator` | role | React Native 앱 빌드 |
| `harness-evaluator` | role | QA 검증 (PASS/FAIL) |
| `harness-admob` | role | AdMob 광고 통합 |
| `harness-build` | role | EAS Build |
| `harness-screenshot` | role | Maestro 스크린샷 |
| `harness-submit` | role | App Store + Google Play 제출 |
| `harness-status` | utility | 파이프라인 상태 조회 |
| `harness-resume` | utility | 파이프라인 재개 |
...
```bash
/harness "앱 설명"              # 새 파이프라인
/harness --resume               # 재개
/harness --status               # 상태 확인
/harness --status --verbose     # 상세 상태
/harness --rounds 5             # QA 최대 5라운드
/harness --ref https://...      # 참고 사이트
/harness --ref ./mockup.png     # 참고 이미지
/harness --skip-research        # 시장 조사 스킵
/harness --skip-admob           # AdMob 스킵
```
